...
function controllerFor(ui: UI, repo: TempRepo, runner?: FilterRepoRunner): Controller {
	return new Controller({ ui, settings: DEFAULT_SETTINGS, exec: repo.exec, filterRepoRunner: runner });
}
...
describe('controller - cleanHistory flows', () => {
	...
	it('hands over the exact script when git-filter-repo is not installed', async () => {
		const repo = await repoWithDeadPath();
		try {
