...
```bash
# sidebar graph, Timeline, SCM menus, palette
code --install-extension dist/git-easy-context-operations-0.4.5.vsix

# plus the context menus in the built-in Source Control Graph
code --install-extension dist/git-easy-context-operations-0.4.5+graph.vsix
```

(`codium --install-extension ...` works the same way.) For the `+graph` build,
allow the proposed API once - inside the editor run **Git Easy Ops: Enable Source
Control Graph Menu...** and pick `product.json` or `argv.json`, or edit the file
yourself and restart:

```jsonc
// ~/.vscode/argv.json (~/.vscode-oss/argv.json for VSCodium)
...
One command, on Linux, macOS and Windows - it installs what is missing and
produces both `.vsix` files in `dist/`:
...
```bash
npm install
npm run package         # publishable build
npm run package:graph   # + the Source Control Graph context menus
```

`npm run package:graph` patches `package.json` temporarily and always restores it -
never publish a manifest that declares `enabledApiProposals`.

## Development

```bash
python3 build.py --tests   # or: npm ci && npm test && npm run package
npm run test:vscode        # integration smoke test inside a real VS Code
```

`src/core/` is the git engine (pure TypeScript, injectable process spawner, UI
interface - that is why it is testable without an editor); `src/vscode/` and
`src/extension.ts` are the thin adapter.

# Git Easy Ops - full reference

The deep detail that used to fill `README.md`: every command, every menu
surface, the settings table, the known limits and the release checklist. Kept
out of the Marketplace listing, which shows the short `README.md` instead.

Repository: https://github.com/luncat8/git-easy-context-operations
...
1. install the graph build (`dist/git-easy-context-operations-0.4.5+graph.vsix`), and
...
# sidebar graph, Timeline, SCM menus, palette
code   --install-extension dist/git-easy-context-operations-0.4.5.vsix
codium --install-extension dist/git-easy-context-operations-0.4.5.vsix

# recommended graph: the same plus context menus in the built-in Source Control Graph
code   --install-extension dist/git-easy-context-operations-0.4.5+graph.vsix
codium --install-extension dist/git-easy-context-operations-0.4.5+graph.vsix
```

For the recommended graph, allow the proposed API once - inside the editor run **Git
...
`dist/git-easy-context-operations-0.4.5.vsix` (no `+graph`) is the Marketplace-safe
